...
class MappingsService:
    ...
    def get_mappings_by_systems(
        ...
        """
        mapping_type_enum = MappingType(mapping_type.lower())
        source_enum = DataSystem(source_system.lower())
        target_enum = DataSystem(target_system.lower())

        if mapping_type_enum == MappingType.stratigraphy:
        ...
        filtered = [
            ...
            if mapping.source_system == source_enum
            and mapping.target_system == target_enum
        ]
    ...
    def update_mappings_by_systems(
        ...
        for mapping in new_mappings:
            if mapping.mapping_type != mapping_type_enum:
                raise ValueError(
            ...
            if mapping.target_system != target_enum:
                raise ValueError(
        ...
        if mapping_type_enum == MappingType.stratigraphy:
            ...
            other_mappings = [
                ...
                for mapping in all_mappings
                if not (
                    mapping.source_system == source_enum
                    and mapping.target_system == target_enum
                )
            ]
